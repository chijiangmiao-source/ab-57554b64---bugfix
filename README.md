...
1. 后端单元测试（pytest，含目标层级、同优范围、字典序、冲突核、大整数精确
   往返、HTTP 用例）；
2. 前端生产构建（vite build；另用 esbuild 把同一份 App 源码打成 jsdom 可执行
   的 IIFE，不改变交付产物），以及大整数 JSON 解析/序列化单元测试
   （node --test，覆盖请求提交与响应展示两个方向）；
3. HTTP 冒烟（API `/health`、Web 首页、nginx 代理的 `/health`、静态资源、meta）；
4. 对**真实 API** 用四类样例核对：共享上游补偿、同优边范围、叶端窗口冲突、
   超出 JS 安全整数的大整数批次精确往返；
5. 对**真实构建页面**驱动 React UI（加载样例→提交→真实请求→断言渲染的树表/
   边表/叶端表/冲突面板），核对 422 与 JSON 错误后输入保留，并核对大整数
   批次（9007199254740993）经真实提交路径在请求与响应两个方向均无静默改写。
